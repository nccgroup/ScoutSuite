...
const Table = props => {
  const { columns, data, initialState = {}, formatters = {}, sortBy = {} } = props;
  ...
  const cols = columns.map((item) => {
    ...
    if (sortBy[item.key]) col.sortType = typeof sortBy[item.key] === 'function' ? sortBy[item.key] : sortBy[item.key];
    return col;
  });

...
testcases = [
    {"name": "account_id", "regex": re_account_id},
    {"name": "ip_ranges_from_file", "regex": re_ip_ranges_from_file},
    {"name": "ip_ranges_from_local_file", "regex": re_ip_ranges_from_local_file},
]
...
class Rule(object):
    ...
    def set_definition(
        self, rule_definitions, attributes=None, ip_ranges=None, params=None
    ):
        """
        ...
        try:
                ...
                if condition[0].startswith("_INCLUDE_("):
                    ...
                    with open(rules_path, "rt") as f:
                        ...
                        for (i, value) in enumerate(condition[1]):
                            new_conditions = re.sub(
                                condition[1][i], condition[2][i], new_conditions
                            )
            ...
            for condition in definition["conditions"]:
                ...
                for testcase in testcases:
                    ...
                    if result and (
                        testcase["name"] == "ip_ranges_from_file"
                        or testcase["name"] == "ip_ranges_from_local_file"
                    ):
                        filename = result.groups()[0]
                        conditions = (
                            result.groups()[1] if len(result.groups()) > 1 else []
                        )
                        ...
                        if filename == ip_ranges_from_args:
                            ...
                            local_file = (
                                True
                                if testcase["name"] == "ip_ranges_from_local_file"
                                else False
                            )

...
class ScoutSuiteArgumentParser:
    def __init__(self):
        ...
        self.parser.add_argument(
            "--provider", action="store_true", dest="sinkhole", help=argparse.SUPPRESS
        )
        ...
        self.subparsers = self.parser.add_subparsers(
            title="The provider you want to run scout against", dest="provider"
        )
    ...
    def _init_aws_parser(self):
        ...
        aws_additional_parser.add_argument(
            "-r",
            "--regions",
            dest="regions",
            default=[],
    ...
    def _init_gcp_parser(self):
        ...
        gcp_scope.add_argument(
            "--project-id", action="store", help="ID of the GCP Project to scan"
        )

        gcp_scope.add_argument(
            "--folder-id", action="store", help="ID of the GCP Folder to scan"
        )
    ...
    def _init_azure_parser(self):
        ...
        azure_auth_params.add_argument(
            "-u",
            "--username",
            action="store",
            default=None,
    ...
    def _init_aliyun_parser(self):
        ...
        aliyun_auth_modes.add_argument(
            "--access-keys", action="store_true", help="Run Scout with user credentials"
        )

        aliyun_auth_params.add_argument(
            "-k",
            "--access-key-id",
            action="store",
            default=None,
    ...
    def _init_oci_parser(self):
        ...
        parser.add_argument(
            "-p", "--profile", dest="profile", default=None, help="Name of the profile"
        )
    ...
    def parse_args(self, args=None):
        ...
        if v.get("provider") == "aws":
            if v.get("aws_access_keys") and not (
                v.get("aws_access_key_id") or v.get("aws_secret_access_key")
            ):
                self.parser.error(
                    "When running with --access-keys, you must provide an Access Key ID "
                    "and Secret Access Key."
                )
        # Azure
        elif v.get("provider") == "azure":
            if v.get("tenant_id") and not (
                v.get("service_principal") or v.get("user_account_browser")
            ):
                self.parser.error(
            ...
            if v.get("user_account_browser") and not v.get("tenant_id"):
                self.parser.error(
                    "You must provide --tenant when using --user-account-browser authentication"
                )
            if v.get("subscription_ids") and v.get("all_subscriptions"):
                self.parser.error(
                    "--subscription-ids and --all-subscriptions are mutually exclusive options"
                )

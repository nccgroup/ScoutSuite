...
def pass_conditions(
    all_info, current_path, conditions, unknown_as_pass_condition=False
):
    """
    ...
    for condition in conditions:
        if condition[0] in ["and", "or"]:
            res = pass_conditions(
                all_info, current_path, condition, unknown_as_pass_condition
            )
        else:
            ...
            if type(test_values) != list:
                ...
                if dynamic_value:
                    test_values = get_value_at(
                        all_info, current_path, dynamic_value.groups()[0], True
                    )
            try:
                ...
                print_exception(
                    "Unable to process testcase '%s' on value '%s', interpreted as %s: %s"
...
def __prepare_age_test(a, b):
    if type(a) != list:
        print_error(
            "Error: olderThan requires a list such as [ N , 'days' ] or [ M, 'hours']."
        )
        raise Exception
    number = int(a[0])
    unit = a[1]

...
def recurse(
    all_info, current_info, target_path, current_path, config, add_suffix=False
):
    """
    ...
    # Python 2-3 compatible way to check for string type
        ...
        split_current_path = copy.deepcopy(current_path)
        results = results + recurse(
            all_info, current_info, [], split_current_path, config, add_suffix
        )
    else:
        print_error(
            "Error: unhandled case, typeof(current_info) = %s" % type(current_info)
        )

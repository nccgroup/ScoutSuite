...
class ScoutResultEncoder(object):
    ...
    def to_dict(config):
        return json.loads(
            json.dumps(config, separators=(",", ": "), cls=ScoutJsonEncoder)
        )
...
class SqlLiteEncoder(ScoutResultEncoder):
    def load_from_file(self, config_type, config_path=None):
        if not config_path:
            config_path, _ = get_filename(
                config_type, self.report_name, self.report_dir
            )
        return SqliteDict(config_path, autocommit=True).data

    def save_to_file(self, config, config_type, force_write, _debug):
        config_path, first_line = get_filename(
            config_type, self.report_name, self.report_dir, file_extension="db"
        )
...
class JavaScriptEncoder(ScoutResultEncoder):
    ...
    def load_from_file(self, file_type, file_path=None, first_line=None):
        if not file_path:
            file_path, first_line = get_filename(
                file_type, self.report_name, self.report_dir
            )

...
from ScoutSuite import (
    DEFAULT_REPORT_DIRECTORY,
    DEFAULT_REPORT_RESULTS_DIRECTORY,
    DEFAULT_INCLUDES_DIRECTORY,
)
...
class HTMLReport(object):
    ...
    def __init__(
        ...
        self.scout_report_data_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "data"
        )
        self.html_data_path = os.path.join(self.scout_report_data_path, "html")
        self.exceptions_encoder = JavaScriptEncoder(
            self.report_name, report_dir, timestamp
        )
    ...
    def prepare_html_report_dir(self):
        ...
        run_results_dir = os.path.join(
            self.report_dir, DEFAULT_REPORT_RESULTS_DIRECTORY
        )
        ...
        src_inc_scout_dir = os.path.join(
            self.scout_report_data_path, DEFAULT_INCLUDES_DIRECTORY
        )
...
class ScoutReport(HTMLReport):
    ...
    def __init__(
        ...
        super(ScoutReport, self).__init__(
            report_name, report_dir, timestamp, exceptions, result_format
        )
    ...
    def save(self, config, exceptions, force_write=False, debug=False):
        ...
        self.exceptions_encoder.save_to_file(
            exceptions, "EXCEPTIONS", force_write, debug
        )
        if ERRORS_LIST:
            self.exceptions_encoder.save_to_file(
                ERRORS_LIST, "ERRORS", force_write, debug=True
            )

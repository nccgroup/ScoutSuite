    ...
    def _match_policies_and_entities(self):
        for policy in self["policies"]:
            for user in self["users"]:
                ...
                if self["users"][user]["name"] in self["policies"][policy][
                    "entities"
                ].get("users", []):
                    self["users"][user]["policies"].append(
        ...
        for policy in self["policies"]:
            for group in self["groups"]:
                ...
                if self["groups"][group]["name"] in self["policies"][policy][
                    "entities"
                ].get("groups", []):
                    self["groups"][group]["policies"].append(
                        self["policies"][policy]["id"]
                    )
        for policy in self["policies"]:
            for role in self["roles"]:
                ...
                if self["roles"][role]["name"] in self["policies"][policy][
                    "entities"
                ].get("roles", []):
                    self["roles"][role]["policies"].append(
                        self["policies"][policy]["id"]
                    )

...
class Users(AliyunCompositeResources):
    ...
    async def _parse_user(self, raw_user):
        user = {}
        user["identifier"] = raw_user[
            "UserId"
        ]  # required as groups use the name as an ID
        ...
        user["last_login_datetime"] = (
            user_details.get("LastLoginDate")
            if user_details.get("LastLoginDate") != ""
            else None
        )

        user["console_access"] = (
            True if user_details.get("LastLoginDate") else False
        )  # TODO this isn't valid

        # get the MFA status for the user
        mfa_enabled, mfa_serial_number = await self.facade.ram.get_user_mfa_status(
            user["name"]
        )

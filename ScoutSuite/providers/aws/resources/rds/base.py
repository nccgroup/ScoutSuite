...
class RDS(Regions):
    ...
    async def fetch_all(
        self, regions=None, excluded_regions=None, partition_name="aws", **kwargs
    ):
        ...
        for region in self["regions"]:
            ...
            self["regions"][region]["snapshots_count"] = sum(
                [
                    len(vpc["snapshots"])
                    for vpc in self["regions"][region]["vpcs"].values()
                ]
        ...
        self["snapshots_count"] = sum(
            [region["snapshots_count"] for region in self["regions"].values()]
        )
        self["subnet_groups_count"] = sum(
            [region["subnet_groups_count"] for region in self["regions"].values()]
        )

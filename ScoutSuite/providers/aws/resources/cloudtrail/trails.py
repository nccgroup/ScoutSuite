...
class Trails(AWSResources):
    ...
    def _parse_trail(self, raw_trail):
        ...
        if (
            "IsMultiRegionTrail" in raw_trail
            and raw_trail["IsMultiRegionTrail"]
            and raw_trail["HomeRegion"] != self.region
        ):
            for key in ["HomeRegion", "TrailARN"]:
                trail[key] = raw_trail[key]
            trail["scout_link"] = "services.cloudtrail.regions.%s.trails.%s" % (
                raw_trail["HomeRegion"],
                trail_id,
            )
    ...
    def data_logging_status(self, trail):
        for event_selector in trail["EventSelectors"]:
            has_wildcard = (
                {u"Values": ["arn:aws:s3"], "Type": "AWS::S3::Object"}
                in event_selector["DataResources"]
                or {u"Values": ["arn:aws:lambda"], "Type": "AWS::Lambda::Function"}
                in event_selector["DataResources"]
            )
    ...
    def is_fresh(trail_details):
        if trail_details.get("LatestCloudWatchLogsDeliveryTime"):
            delivery_time = trail_details.get(
                "LatestCloudWatchLogsDeliveryTime"
            ).strftime("%s")

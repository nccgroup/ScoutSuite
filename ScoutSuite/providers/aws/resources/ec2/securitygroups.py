...
class SecurityGroups(AWSResources):
    ...
    async def fetch_all(self):
        raw_security_groups = await self.facade.ec2.get_security_groups(
    ...
    def _parse_security_group(self, raw_security_group):
        ...
        if "Tags" in raw_security_group:
            security_group["tags"] = {
                x["Key"]: x["Value"] for x in raw_security_group["Tags"]
            }
        ...
        for rule in rules:
            ...
            if "FromPort" in rule and "ToPort" in rule:
                if ip_protocol == "ICMP":
                    ...
                    port_value = self.icmp_message_types_dict[str(rule["FromPort"])]

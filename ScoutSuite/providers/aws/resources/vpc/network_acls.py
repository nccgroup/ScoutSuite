        ...
        for entry in entries:
            if entry["Egress"] == egress:
                ...
                acl["CidrBlock"] = (
                    ...
                    else entry["Ipv6CidrBlock"]
                )
                acl["protocol"] = protocols_dict[entry["Protocol"]]
                if "PortRange" in entry:
                    from_port = (
                        entry["PortRange"]["From"] if entry["PortRange"]["From"] else 1
                    )
                    to_port = (
                        entry["PortRange"]["To"] if entry["PortRange"]["To"] else 65535
                    )
                    acl["port_range"] = (
                        from_port
                        if from_port == to_port
                        else str(from_port) + "-" + str(to_port)
                    )
                else:

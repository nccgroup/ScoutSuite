...
class IAM(AWSCompositeResources):
    ...
    def _parse_permissions(
        ...
        for statement in policy_document["Statement"]:
            self._parse_statement(
                policy_name, statement, policy_type, iam_resource_type, resource_name
            )

    def _parse_statement(
        self, policy_name, statement, policy_type, iam_resource_type, resource_name
    ):
        # Effect
        ...
        self._parse_actions(
            effect,
            action_string,
            statement[action_string],
            resource_string,
            statement[resource_string],
            iam_resource_type,
            resource_name,
            policy_name,
            policy_type,
    ...
    def _parse_actions(
        ...
        for action in actions:
            ...
            self["permissions"][action_string][action][iam_resource_type][
                effect
            ].setdefault(iam_resource_name, {})
            self._parse_action(
                effect,
                action_string,
                action,
    ...
    def _parse_resource(
        ...
        self["permissions"][action_string][action][iam_resource_type][effect][
            iam_resource_name
        ].setdefault(resource_string, {})
        self["permissions"][action_string][action][iam_resource_type][effect][
            iam_resource_name
        ][resource_string].setdefault(resource, {})
        self["permissions"][action_string][action][iam_resource_type][effect][
            iam_resource_name
        ][resource_string][resource].setdefault(policy_type, {})
        self["permissions"][action_string][action][iam_resource_type][effect][
            iam_resource_name
        ][resource_string][resource][policy_type].setdefault(policy_name, {})
        self["permissions"][action_string][action][iam_resource_type][effect][
            iam_resource_name
        ][resource_string][resource][policy_type][policy_name].setdefault(
            "condition", condition
        )

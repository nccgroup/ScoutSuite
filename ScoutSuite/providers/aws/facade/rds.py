...
class RDSFacade(AWSBaseFacade):
        ...
        async with self._regional_instances_cache_locks.setdefault(region, Lock()):
            ...
            for instance in self._instances_cache[region]:
                instance["VpcId"] = (
                    ...
                    and "VpcId" in instance["DBSubnetGroup"]
    ...
    async def _get_and_set_instance_tags(self, instance: {}, region: str):
        ...
        try:
            instance_tagset = await run_concurrently(
                lambda: client.list_tags_for_resource(
                    ResourceName="arn:aws:rds:"
                    + region
                    + ":"
                    + account_id
                    + ":db:"
            ...
            instance["Tags"] = {
                x["Key"]: x["Value"] for x in instance_tagset["TagList"]
            }
        except ClientError as e:
            if e.response["Error"]["Code"] != "NoSuchTagSet":
                print_exception(
                    "Failed to get db instance tags for %s: %s"
                    % (instance["DBInstanceIdentifier"], e)
                )
        except Exception as e:
            print_exception(
                "Failed to get db instance tags for %s: %s"
    ...
    async def _cache_snapshots(self, region: str):
        async with self._regional_snapshots_cache_locks.setdefault(region, Lock()):
            ...
            for snapshot in self._snapshots_cache[region]:
                snapshot["VpcId"] = (
                    snapshot["VpcId"] if "VpcId" in snapshot else ec2_classic
                )

            await get_and_set_concurrently(
                [self._get_and_set_snapshot_attributes],
                self._snapshots_cache[region],
                region=region,
            )
    ...
    async def _get_and_set_snapshot_attributes(self, snapshot: {}, region: str):
        ...
        try:
            ...
            snapshot["Attributes"] = (
                attributes["DBSnapshotAttributes"]
                if "DBSnapshotAttributes" in attributes
                else {}
            )
        except Exception as e:

...
class EC2Facade(AWSBaseFacade):
    ...
    async def get_ebs_encryption(self, region):
        ec2_client = AWSFacadeUtils.get_client('ec2', self.session, region)
        try:
            encryption_settings = await run_concurrently(lambda: ec2_client.get_ebs_encryption_by_default()['EbsEncryptionByDefault'])
    ...
    async def get_ebs_default_encryption_key(self, region):
        ec2_client = AWSFacadeUtils.get_client('ec2', self.session, region)
        try:
            encryption_key = await run_concurrently(lambda: ec2_client.get_ebs_default_kms_key_id()['KmsKeyId'])

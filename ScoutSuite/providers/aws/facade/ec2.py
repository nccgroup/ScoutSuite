...
class EC2Facade(AWSBaseFacade):
    ...
    async def get_instance_user_data(self, region: str, instance_id: str):
        ...
        try:
            user_data_response = await run_concurrently(
                lambda: ec2_client.describe_instance_attribute(
    ...
    async def get_instances(self, region: str, vpc: str):
        ...
        try:
            reservations = await AWSFacadeUtils.get_all_pages(
                "ec2",
                region,
                self.session,
                "describe_instances",
    ...
    async def get_images(self, region: str):
        ...
        try:
            return await run_concurrently(
                lambda: client.describe_images(Filters=filters)["Images"]
            )
        except Exception as e:
    ...
    async def get_volumes(self, region: str):
        try:
            volumes = await AWSFacadeUtils.get_all_pages(
                "ec2", region, self.session, "describe_volumes", "Volumes"
            )
            await get_and_set_concurrently(
                [self._get_and_set_key_manager], volumes, region=region
            )
            return volumes
        except Exception as e:
        ...
        if "KmsKeyId" in volume:
            ...
            try:
                volume["KeyManager"] = await run_concurrently(
                    lambda: kms_client.describe_key(KeyId=key_id)["KeyMetadata"][
                        "KeyManager"
                    ]
                )
            except Exception as e:
    ...
    async def get_snapshots(self, region: str):
        ...
        try:
            ...
            await get_and_set_concurrently(
                [self._get_and_set_snapshot_attributes], snapshots, region=region
            )
        finally:
            return snapshots
    ...
    async def cache_flow_logs(self, region: str):
        async with self.regional_flow_logs_cache_locks.setdefault(
            region, asyncio.Lock()
        ):
            if region in self.flow_logs_cache:
                return

            self.flow_logs_cache[region] = await AWSFacadeUtils.get_all_pages(
                "ec2", region, self.session, "describe_flow_logs", "FlowLogs"
            )
    ...
    async def get_subnets(self, region: str, vpc: str):
        ...
        try:
            ...
            await get_and_set_concurrently(
                [self._get_and_set_subnet_flow_logs], subnets, region=region
            )
            return subnets
    ...
    async def _get_and_set_subnet_flow_logs(self, subnet: {}, region: str):
        ...
        subnet["flow_logs"] = [
            ...
            if flow_log["ResourceId"] == subnet["SubnetId"]
            or flow_log["ResourceId"] == subnet["VpcId"]
        ]

    async def get_peering_connections(self, region):
        try:
            peering_connections = await AWSFacadeUtils.get_all_pages(
                "ec2",
                region,
                self.session,

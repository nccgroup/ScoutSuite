...
class S3Facade(AWSBaseFacade):
    ...
    async def _get_and_set_s3_bucket_logging(self, bucket):
        ...
        try:
            logging = await run_concurrently(
                lambda: client.get_bucket_logging(Bucket=bucket["Name"])
            )
        except Exception as e:
            print_exception(
                "Failed to get logging configuration for %s: %s" % (bucket["Name"], e)
            )
    ...
    async def _get_and_set_s3_bucket_versioning(self, bucket):
        client = AWSFacadeUtils.get_client("s3", self.session, bucket["region"])
        try:
            versioning = await run_concurrently(
                lambda: client.get_bucket_versioning(Bucket=bucket["Name"])
            )
            bucket["versioning_status_enabled"] = self._status_to_bool(
                versioning.get("Status")
            )
            bucket["version_mfa_delete_enabled"] = self._status_to_bool(
                versioning.get("MFADelete")
            )
        except Exception as e:
            print_exception(
                "Failed to get versioning configuration for %s: %s"
                % (bucket["Name"], e)
            )
    ...
    async def _get_and_set_s3_bucket_webhosting(self, bucket):
        ...
        try:
            result = await run_concurrently(
                lambda: client.get_bucket_website(Bucket=bucket["Name"])
            )
            ...
            if "NoSuchWebsiteConfiguration" in str(e):
                ...
                print_exception(
                    "Failed to get web hosting configuration for %s: %s"
                    % (bucket["Name"], e)
                )
    ...
    async def _get_and_set_s3_acls(self, bucket, key_name=None):
        ...
        try:
            ...
            print_exception(
                "Failed to get ACL configuration for %s: %s" % (bucket_name, e)
            )
            bucket["grantees"] = {}
    ...
    async def _get_and_set_s3_bucket_policy(self, bucket):
        client = AWSFacadeUtils.get_client("s3", self.session, bucket["region"])
        try:
            bucket_policy = await run_concurrently(
                lambda: client.get_bucket_policy(Bucket=bucket["Name"])
            )
            bucket["policy"] = json.loads(bucket_policy["Policy"])
        except ClientError as e:
            if e.response["Error"]["Code"] != "NoSuchBucketPolicy":
                print_exception(
                    "Failed to get bucket policy for %s: %s" % (bucket["Name"], e)
                )
        except Exception as e:
            print_exception(
                "Failed to get bucket policy for %s: %s" % (bucket["Name"], e)
    ...
    async def _get_and_set_s3_bucket_tags(self, bucket):
        client = AWSFacadeUtils.get_client("s3", self.session, bucket["region"])
        try:
            bucket_tagset = await run_concurrently(
                lambda: client.get_bucket_tagging(Bucket=bucket["Name"])
            )
            bucket["tags"] = {x["Key"]: x["Value"] for x in bucket_tagset["TagSet"]}
        except ClientError as e:
            if e.response["Error"]["Code"] != "NoSuchTagSet":
                print_exception(
                    "Failed to get bucket tags for %s: %s" % (bucket["Name"], e)
                )
        except Exception as e:
            print_exception(
                "Failed to get bucket tags for %s: %s" % (bucket["Name"], e)
    ...
    def _set_s3_bucket_secure_transport(self, bucket):
        try:
            if "policy" in bucket:
                ...
                for statement in bucket["policy"]["Statement"]:
                    ...
                    if (
                        ...
                        and "Bool" in statement["Condition"]
            ...
            print_exception(
                "Failed to evaluate bucket policy for %s: %s" % (bucket["Name"], e)
            )

...
class CloudFormation(AWSBaseFacade):
    async def get_stacks(self, region: str):
        try:
            ...
            stacks = [
                stack for stack in stacks if not CloudFormation._is_stack_deleted(stack)
            ]
            await get_and_set_concurrently(
                [
                    self._get_and_set_description,
                    self._get_and_set_template,
                    self._get_and_set_policy,
                ],
                stacks,
                region=region,
            )
    ...
    async def _get_and_set_template(self, stack: {}, region: str):
        ...
        try:
            stack["template"] = await run_concurrently(
                lambda: client.get_template(StackName=stack["StackName"])[
                    "TemplateBody"
                ]
            )
        except Exception as e:

...
class SNSFacade(AWSBaseFacade):
    ...
    async def get_topics(self, region: str):
        try:
            topics = await AWSFacadeUtils.get_all_pages(
                "sns", region, self.session, "list_topics", "Topics"
            )
            await get_and_set_concurrently(
                [self._get_and_set_topic_attributes], topics, region=region
            )
    ...
    async def _get_and_set_topic_attributes(self, topic: {}, region: str):
        ...
        try:
            topic["attributes"] = await run_concurrently(
                lambda: sns_client.get_topic_attributes(TopicArn=topic["TopicArn"])[
                    "Attributes"
                ]
            )
        except Exception as e:

    ...
    async def get_identities(self, region: str):
        try:
            ...
            return await map_concurrently(
                self._get_identity_dkim_attributes, identity_names, region=region
            )
    ...
    async def get_identity_policies(self, region: str, identity_name: str):
        ...
        try:
            policy_names = await run_concurrently(
                lambda: ses_client.list_identity_policies(Identity=identity_name)[
                    "PolicyNames"
                ]
            )
        except Exception as e:
        ...
        try:
            return await run_concurrently(
                lambda: ses_client.get_identity_policies(
                    Identity=identity_name, PolicyNames=policy_names
                )["Policies"]
            )
        except Exception as e:

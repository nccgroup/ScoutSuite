...
class IAMFacade(AWSBaseFacade):
    async def get_credential_reports(self):
        ...
        try:
            while not report_generated and n_attempts > 0:
                ...
                if response["State"] == "COMPLETE":
                    ...
                    await asyncio.sleep(
                        0.1
                    )  # Wait for 100ms before doing a new attempt.
            ...
            if not report_generated and n_attempts == 0:
                print_exception(
                    "Failed to complete credential report generation in {} attempts".format(
                        n_attempts
                    )
                )
                return []
        ...
        try:
            report = await run_concurrently(
                lambda: client.get_credential_report()["Content"]
            )
    ...
    async def get_groups(self):
        groups = await AWSFacadeUtils.get_all_pages(
            "iam", None, self.session, "list_groups", "Groups"
        )
        await get_and_set_concurrently(
            [
                self._get_and_set_group_users,
                functools.partial(
                    self._get_and_set_inline_policies, iam_resource_type="group"
                ),
            ],
            groups,
        )
    ...
    async def get_users(self):
        users = await AWSFacadeUtils.get_all_pages(
            "iam", None, self.session, "list_users", "Users"
        )
        await get_and_set_concurrently(
            [
                functools.partial(
                    self._get_and_set_inline_policies, iam_resource_type="user"
                ),
    ...
    async def get_roles(self):
        roles = await AWSFacadeUtils.get_all_pages(
            "iam", None, self.session, "list_roles", "Roles"
        )
        for role in roles:
        ...
        await get_and_set_concurrently(
            [
                functools.partial(
                    self._get_and_set_inline_policies, iam_resource_type="role"
                ),
    ...
    async def get_password_policy(self):
        client = AWSFacadeUtils.get_client("iam", self.session)
        try:
            return (await run_concurrently(client.get_account_password_policy))[
                "PasswordPolicy"
            ]
        except ClientError as e:
            if e.response["Error"]["Code"] != "NoSuchEntity":
    ...
    async def _get_and_set_group_users(self, group: {}):
        ...
        try:
            users = await run_concurrently(
                lambda: client.get_group(GroupName=group["GroupName"])["Users"]
            )
            group["Users"] = [user["UserId"] for user in users]
        except Exception as e:
            print_exception(
                "Failed to get IAM group {}: {}".format(group["GroupName"], e)
            )
            ...
            try:
                tasks = {
                    asyncio.ensure_future(
                        run_concurrently(
                            lambda policy_name=policy_name: get_policy_method(
                                **dict(args, PolicyName=policy_name)
                            )
                        )
                    )
                ...
                for task in asyncio.as_completed(tasks):
                    ...
                    resource["inline_policies"][policy_id][
                        "PolicyDocument"
                    ] = self._normalize_statements(policy_document)
    ...
    def _normalize_statements(self, policy_document):
        if policy_document:
            if type(policy_document["Statement"]) == list:
                ...
                policy_document["Statement"] = self._normalize_single_statement(

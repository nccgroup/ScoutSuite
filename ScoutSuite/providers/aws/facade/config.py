...
class ConfigFacade(AWSBaseFacade):
    async def get_rules(self, region):
        try:
            return await AWSFacadeUtils.get_all_pages(
                "config", region, self.session, "describe_config_rules", "ConfigRules"
            )
    ...
    async def get_recorders(self, region: str):
        ...
        try:
            recorders = (
                await run_concurrently(client.describe_configuration_recorders)
        ...
        try:
            ...
            recorder_statuses_map = {
                recorder["name"]: recorder for recorder in recorder_statuses_list
            }
            for recorder in recorders:
                recorder["ConfigurationRecordersStatus"] = recorder_statuses_map[
                    recorder["name"]
                ]

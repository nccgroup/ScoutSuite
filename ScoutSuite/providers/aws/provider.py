...
class AWSProvider(BaseProvider):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(AWSProvider, self).__init__(
            report_dir, timestamp, services, skipped_services, result_format
        )
    ...
    def preprocessing(self, ip_ranges=None, ip_ranges_name_key=None):
        ...
        if (
            "emr" in self.service_list
            and "ec2" in self.service_list
            and "vpc" in self.service_list
        ):
    ...
    def _add_security_group_data_to_elbv2(self):
        def check_security_group_rules(lb, index, traffic_type):
            ...
            for protocol in lb["security_groups"][index]["rules"][traffic_type][
                "protocols"
            ]:
                for port in lb["security_groups"][index]["rules"][traffic_type][
                    "protocols"
                ][protocol]["ports"]:
            ...
            for vpc in elbv2_config["regions"][region]["vpcs"]:
                for lb in elbv2_config["regions"][region]["vpcs"][vpc]["lbs"]:
                    for i in range(
                        0,
                        len(
                            elbv2_config["regions"][region]["vpcs"][vpc]["lbs"][lb][
                                "security_groups"
                            ]
                        ),
                    ):
                        for sg in ec2_config["regions"][region]["vpcs"][vpc][
                            "security_groups"
                        ]:
                            group_id = elbv2_config["regions"][region]["vpcs"][vpc][
                                "lbs"
                            ][lb]["security_groups"][i]["GroupId"]
                            if (
                                "GroupId"
                                in elbv2_config["regions"][region]["vpcs"][vpc]["lbs"][
                                    lb
                                ]["security_groups"][i]
                                and group_id == sg
                            ):
                                elbv2_config["regions"][region]["vpcs"][vpc]["lbs"][lb][
                                    "security_groups"
                                ][i] = ec2_config["regions"][region]["vpcs"][vpc][
                                    "security_groups"
                                ][
                                    sg
                                ]
    ...
    def _add_last_snapshot_date_to_ec2_volumes(self):
        for region in self.services["ec2"]["regions"].values():
            for volumeId, volume in region.get("volumes").items():
                completed_snapshots = [
                    s
                    for s in region["snapshots"].values()
                    if s["VolumeId"] == volumeId and s["State"] == "completed"
                ]
                sorted_snapshots = sorted(
                    completed_snapshots, key=lambda s: s["StartTime"], reverse=True
                )
                volume["LastSnapshotDate"] = (
                    sorted_snapshots[0]["StartTime"]
                    if len(sorted_snapshots) > 0
                    else None
                )

    def add_security_group_name_to_ec2_grants_callback(
        self, current_config, path, current_path, ec2_grant, callback_args
    ):
        ...
        if sg_id in current_path:
            ...
            "UserId" in ec2_grant
            and ec2_grant["UserId"] == callback_args["AWSAccountId"]
        ):
    ...
    def process_network_acls_callback(
        self, current_config, path, current_path, privateip_id, callback_args
    ):
    ...
    def _process_network_acls_check_for_aws_default(network_acl, direction):
        if (
            len(network_acl["rules"][direction]) == 2
            and int(network_acl["allow_all_%s_traffic" % direction]) > 0
            and "100" in network_acl["rules"][direction]
        ):
    ...
    def list_ec2_network_attack_surface_callback(
        self, current_config, path, current_path, privateip_id, callback_args
    ):
        ...
        if "Association" in current_config and current_config["Association"]:
            ...
            self._security_group_to_attack_surface(
                self.services["ec2"]["external_attack_surface"],
                public_ip,
                current_path,
        ...
        # IPv6
        if (
            "Ipv6Addresses" in current_config
            and len(current_config["Ipv6Addresses"]) > 0
        ):
    ...
    def _match_iam_policies_and_buckets(self):
        ...
        if "Action" in iam_info["permissions"]:
            for action in (
                x
                for x in iam_info["permissions"]["Action"]
                if ((x.startswith("s3:") and x != "s3:ListAllMyBuckets") or (x == "*"))
            ):
                for iam_entity in iam_info["permissions"]["Action"][action]:
                    if "Allow" in iam_info["permissions"]["Action"][action][iam_entity]:
                        for allowed_iam_entity in iam_info["permissions"]["Action"][
                            action
                        ][iam_entity]["Allow"]:
                            # For resource statements, we can easily rely on the existing permissions structure
                            if (
                                "Resource"
                                in iam_info["permissions"]["Action"][action][
                                    iam_entity
                                ]["Allow"][allowed_iam_entity]
                            ):
                                for full_path in (
                                    x
                                    for x in iam_info["permissions"]["Action"][action][
                                        iam_entity
                                    ]["Allow"][allowed_iam_entity]["Resource"]
                                    if x.startswith("arn:aws:s3:") or x == "*"
                                ):
                                    ...
                                    self._update_iam_permissions(
                                        ...
                                        iam_entity,
                                        allowed_iam_entity,
                                        iam_info["permissions"]["Action"][action][
                                            iam_entity
                                        ]["Allow"][allowed_iam_entity]["Resource"][
                                            full_path
                                        ],
                            ...
                            if (
                                ...
                                for full_path in (
                                    ...
                                    for x in iam_info["permissions"]["Action"][action][
                                        iam_entity
                                    ]["Allow"][allowed_iam_entity]["NotResource"]
                                    if x.startswith("arn:aws:s3:") or x == "*"
                                ):
                                    for policy_type in [
                                        "InlinePolicies",
                                        ...
                                        if (
                                            policy_type
                                            in iam_info["permissions"]["Action"][
                                                action
                                            ][iam_entity]["Allow"][allowed_iam_entity][
                                                "NotResource"
                                            ][
                                                full_path
                                            ]
                                        ):
                                            for policy in iam_info["permissions"][
                                                "Action"
                                            ][action][iam_entity]["Allow"][
                                                allowed_iam_entity
                                            ][
                                                "NotResource"
                                            ][
                                                ...
                                                self._update_bucket_permissions(
                                                    s3_info,
                                                    iam_info,
                                                    action,
                                                    iam_entity,
    ...
    def _update_bucket_permissions(
        self,
        s3_info,
        iam_info,
        action,
        ...
        if policy_type == "InlinePolicies":
            policy = iam_info[iam_entity.title()][allowed_iam_entity]["Policies"][
                policy_name
            ]["PolicyDocument"]
        ...
        policy_info[policy_type][policy_name] = iam_info["permissions"]["Action"][
            action
        ][iam_entity]["Allow"][allowed_iam_entity]["NotResource"][full_path][
            policy_type
        ][
            policy_name
        ]
    ...
    def _update_iam_permissions(
        self, s3_info, bucket_name, iam_entity, allowed_iam_entity, policy_info
    ):
        if self.services.get("s3") and self.services.get(
            "iam"
        ):  # validate both services were included in run
            if bucket_name != "*" and bucket_name in s3_info["buckets"]:
                ...
                if "policies" in policy_info:
                    manage_dictionary(
                        bucket[iam_entity][allowed_iam_entity], "policies", {}
                    )
    ...
    def match_network_acls_and_subnets_callback(
        self, current_config, path, current_path, acl_id, callback_args
    ):
    ...
    def match_instances_and_subnets_callback(
        self, current_config, path, current_path, instance_id, callback_args
    ):
        if self.services.get("ec2") and self.services.get(
            "vpc"
        ):  # validate both services were included in run
            subnet_id = current_config["SubnetId"]
            if subnet_id:
                vpc = self.subnet_map[subnet_id]
                subnet = self.services["vpc"]["regions"][vpc["region"]]["vpcs"][
                    vpc["vpc_id"]
                ]["subnets"][subnet_id]
                    ...
                    if "instances" in ec2_config["regions"][r]["vpcs"][v]:
                        for i in ec2_config["regions"][r]["vpcs"][v]["instances"]:
                            instance_profile = ec2_config["regions"][r]["vpcs"][v][
                                "instances"
                            ][i]["IamInstanceProfile"]
                            instance_profile_id = (
                                instance_profile["Id"] if instance_profile else None
                            )
            ...
            for role_id in iam_config["roles"]:
                ...
                for instance_profile_id in iam_config["roles"][role_id][
                    ...
                    if instance_profile_id in role_instances:
                        iam_config["roles"][role_id]["instance_profiles"][
                            instance_profile_id
                        ]["instances"] = role_instances[instance_profile_id]
                        iam_config["roles"][role_id]["instances_count"] += len(
                            role_instances[instance_profile_id]
                        )

    def process_vpc_peering_connections_callback(
        self, current_config, path, current_path, pc_id, callback_args
    ):

        # Create a list of peering connection IDs in each VPC
        info = (
            "AccepterVpcInfo"
            if current_config["AccepterVpcInfo"]["OwnerId"] == self.account_id
            else "RequesterVpcInfo"
        )
        ...
        current_config["peer_info"] = copy.deepcopy(
            current_config[
                "AccepterVpcInfo" if info == "RequesterVpcInfo" else "RequesterVpcInfo"
            ]
        )
        ...
        if (
            ...
            and current_config["peer_info"]["OwnerId"] in self.organization
        ):
            current_config["peer_info"]["name"] = self.organization[
                current_config["peer_info"]["OwnerId"]
            ]["Name"]
        else:
            current_config["peer_info"]["name"] = current_config["peer_info"]["OwnerId"]

    def match_roles_and_cloudformation_stacks_callback(
        self, current_config, path, current_path, stack_id, callback_args
    ):
    ...
    def match_roles_and_vpc_flowlogs_callback(
        self, current_config, path, current_path, flowlog_id, callback_args
    ):
    ...
    def _get_role_info(self, attribute_name, attribute_value):
        ...
        for role_id in self.services["iam"]["roles"]:
            if (
                self.services["iam"]["roles"][role_id][attribute_name]
                == attribute_value
            ):
    ...
    def match_security_groups_and_resources_callback(
        self, current_config, path, current_path, resource_id, callback_args
    ):
        ...
        if "resource_id_path" not in callback_args:
            ...
            resource_path = combine_paths(
                copy.deepcopy(current_path), callback_args["resource_id_path"]
            )
            resource_id = resource_path[-1]
            resource_type = resource_path[-2]
        if "status_path" in callback_args:
            status_path = combine_paths(
                copy.deepcopy(original_resource_path), callback_args["status_path"]
            )
        ...
        try:
            ...
            for resource_sg in sg_attribute:
                ...
                if resource_status:
                    ...
                    if (
                        resource_id
                        not in sg["used_by"][service]["resource_type"][resource_type][
                            resource_status
                        ]
                    ):
                        sg["used_by"][service]["resource_type"][resource_type][
    ...
    def set_emr_vpc_ids_callback(
        self, current_config, path, current_path, vpc_id, callback_args
    ):
    ...
    def sort_vpc_flow_logs_callback(
        self, current_config, path, current_path, flow_log_id, callback_args
    ):
    ...
    def get_db_attack_surface(
        self, current_config, path, current_path, db_id, callback_args
    ):
        service = current_path[1]
        service_config = self.services[service]
        manage_dictionary(service_config, "external_attack_surface", {})
        if (
            (service == "redshift" or service == "rds")
            and "PubliclyAccessible" in current_config
            and current_config["PubliclyAccessible"]
        ):
            ...
            self._security_group_to_attack_surface(
                service_config["external_attack_surface"],
                public_dns,
                current_path,
    ...
    def get_lb_attack_surface(
        self, current_config, path, current_path, elb_id, callback_args
    ):
        ...
        if current_path[1] == "elbv2" and current_config["Type"] == "network":
            ...
            for listener in current_config["listeners"]:
                ...
                elb_config["external_attack_surface"][public_dns]["protocols"][
                    protocol
                ]["ports"][listener]["cidrs"].append({"CIDR": "0.0.0.0/0"})
        elif (
            current_path[1] == "elbv2" and current_config["Scheme"] == "internet-facing"
        ):
            elb_config["external_attack_surface"][public_dns] = {"protocols": {}}
            ...
            self._security_group_to_attack_surface(
                elb_config["external_attack_surface"],
                public_dns,
                current_path,
                security_groups,
                listeners,
            ...
            for listener in current_config["listeners"]:
                ...
                elb_config["external_attack_surface"][public_dns]["protocols"]["TCP"][
                    "ports"
                ][listener]["cidrs"].append({"CIDR": "0.0.0.0/0"})

    def _security_group_to_attack_surface(
        self,
        ...
        for sg_id in security_groups:
            ...
            for p in ingress_rules["protocols"]:
                for port in ingress_rules["protocols"][p]["ports"]:
                    if (
                        len(listeners) == 0
                        and "cidrs" in ingress_rules["protocols"][p]["ports"][port]
                    ):
                        manage_dictionary(
                        ...
                        attack_surface_config[public_ip]["protocols"][p]["ports"][port][
                            "cidrs"
                        ...
                        for listener in listeners:
                            if (
                                ...
                                and port_min < int(listener) < port_max
                                and "cidrs"
                                in ingress_rules["protocols"][p]["ports"][port]
                            ):
                                manage_dictionary(
                                    attack_surface_config[public_ip]["protocols"],
                                    p,
                                ...
                                attack_surface_config[public_ip]["protocols"][p][
                                    "ports"
                                ][str(listener)]["cidrs"] += ingress_rules["protocols"][
                                    p
                                ][
                                    "ports"
                                ][
    ...
    def _parse_elb_policies(self):
        self._go_to_and_do(
            self.services["elb"], ["regions"], [], self.parse_elb_policies_callback, {}
        )

    def parse_elb_policies_callback(
        self, current_config, path, current_path, region_id, callback_args
    ):
        region_config = get_object_at(
            self, ["services", "elb",] + current_path + [region_id]
        )
        region_config["elb_policies"] = current_config["elb_policies"]
        for policy_id in region_config["elb_policies"]:
            if (
                region_config["elb_policies"][policy_id]["PolicyTypeName"]

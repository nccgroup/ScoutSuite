...
try:
    ...
    pass
try:
    from ScoutSuite.providers.azure.facade.loadbalancer_private import (
        LoadBalancerFacade,
    )
except ImportError:
    pass
    ...
    def _set_subscriptions(self):
        ...
        if not (self.subscription_ids or self.all_subscriptions):
            try:
                ...
                print_info(
                    'Running against the "{}" subscription'.format(s.subscription_id)
                )
        ...
        # A specific set of subscriptions
            ...
            subscriptions_list = [
                s
                for s in accessible_subscriptions_list
                if s.subscription_id in self.subscription_ids
            ]
            # Verbose skip
            for s in self.subscription_ids:
                if not any(
                    subs.subscription_id == s for subs in accessible_subscriptions_list
                ):
                    print_info(
            ...
            print_info(
                "Running against {} subscription(s)".format(len(subscriptions_list))
            )

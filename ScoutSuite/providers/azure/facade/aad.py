...
class AADFacade:
    def __init__(self, credentials):
        self._client = GraphRbacManagementClient(
            credentials.graph_credentials, tenant_id=credentials.get_tenant_id()
        )
    ...
    async def get_user_groups(self, user_id):
        try:
            return await run_concurrently(
                lambda: list(
                    self._client.users.get_member_groups(
                        object_id=user_id, security_enabled_only=False
                    )
                )
            )
        except Exception as e:
    ...
    async def get_service_principals(self):
        try:
            return await run_concurrently(
                lambda: list(self._client.service_principals.list())
            )
        except Exception as e:
    ...
    async def get_applications(self):
        try:
            return await run_concurrently(
                lambda: list(self._client.applications.list())
            )
        except Exception as e:

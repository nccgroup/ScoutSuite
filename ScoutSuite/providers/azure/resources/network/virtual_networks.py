...
class VirtualNetworks(AzureResources):
    ...
    def _parse_virtual_network(self, raw_virtual_network):
        ...
        ] = raw_virtual_network.enable_vm_protection
        virtual_network_dict["etag"] = str(raw_virtual_network.etag)
        virtual_network_dict["tags"] = raw_virtual_network.tags
        virtual_network_dict[
            "virtual_network_peerings"
        ] = raw_virtual_network.virtual_network_peerings
        virtual_network_dict[
            "enable_ddos_protection"
        ] = raw_virtual_network.enable_ddos_protection
        virtual_network_dict["resource_guid"] = raw_virtual_network.resource_guid
        virtual_network_dict[
            "provisioning_state"
        ] = raw_virtual_network.provisioning_state
        virtual_network_dict["address_space"] = raw_virtual_network.address_space
        virtual_network_dict[
            "ddos_protection_plan"
        ] = raw_virtual_network.ddos_protection_plan
        virtual_network_dict["additional_properties"] = list(
            raw_virtual_network.additional_properties
        )
        ...
        for raw_subnet in raw_virtual_network.subnets:
            ...
            ] = raw_subnet.service_association_links
            subnet_dict[
                "resource_navigation_links"
            ] = raw_subnet.resource_navigation_links
            subnet_dict[
                "service_endpoint_policies"
            ] = raw_subnet.service_endpoint_policies
            subnet_dict["interface_endpoints"] = (
                raw_subnet.interface_endpoints
                if hasattr(raw_subnet, "interface_endpoints")
                else None
            )
            ...
            ] = raw_subnet.ip_configuration_profiles
            ...
            if raw_subnet.network_security_group:
                subnet_dict["network_security_group"] = get_non_provider_id(
                    raw_subnet.network_security_group.id
                )
            else:

...
class ApplicationSecurityGroups(AzureResources):
    ...
    async def fetch_all(self):
        for raw_group in await self.facade.network.get_application_security_groups(
            self.subscription_id
        ):
            id, application_security_group = self._parse_application_security_group(
                raw_group
            )
    ...
    def _parse_application_security_group(self, raw_application_security_group):
        ...
        application_security_group_dict[
            "location"
        ] = raw_application_security_group.location
        application_security_group_dict["tags"] = raw_application_security_group.tags
        application_security_group_dict[
            "resource_guid"
        ] = raw_application_security_group.resource_guid
        application_security_group_dict[
            "provisioning_state"
        ] = raw_application_security_group.provisioning_state

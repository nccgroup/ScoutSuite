...
class SecurityGroups(AzureResources):
    ...
    def _parse_network_security_group(self, network_security_group):
        ...
        network_security_group_dict["id"] = get_non_provider_id(
        ...
        network_security_group_dict[
            "provisioning_state"
        ] = network_security_group.provisioning_state
        network_security_group_dict[
            "resource_guid"
        ] = network_security_group.resource_guid
        ...
        network_security_group_dict["security_rules"] = self._parse_security_rules(
        ...
        if network_security_group.network_interfaces:
            for network_interface in network_security_group.network_interfaces:
                ...
                network_security_group_dict["network_interfaces"][identifier] = {
                    "id": identifier
                }
    ...
    def _parse_security_rules(self, network_security_group):
        ...
        for sr in network_security_group.default_security_rules:
            security_rule_id, security_rule_dict = self._parse_security_rule(
                sr, default=True
            )
            security_rules[security_rule_id] = security_rule_dict
    ...
    def _parse_security_rule(self, rule, default=False):
        ...
        source_port_ranges = self._merge_prefixes_or_ports(
        ...
        destination_address_prefixes = self._merge_prefixes_or_ports(
        ...
        destination_port_ranges = self._merge_prefixes_or_ports(
        ...
        security_rule_dict["destination_ports"] = self._parse_ports(
    ...
    def _parse_exposed_ports(self, network_security_group):
        ...
        # Sort by priority.
        rules = (
            network_security_group.default_security_rules
            + network_security_group.security_rules
        )
        rules.sort(key=lambda x: x.priority, reverse=True)

        for sr in rules:
            if sr.direction == "Inbound" and (
                sr.source_address_prefix == "*"
                or sr.source_address_prefix == "Internet"
            ):
                port_ranges = self._merge_prefixes_or_ports(
                    sr.destination_port_range, sr.destination_port_ranges
                )

...
class StorageAccounts(AzureCompositeResources):
    _children = [(BlobContainers, "blob_containers")]
    ...
    async def fetch_all(self):
        for (
            raw_storage_account
        ) in await self.facade.storageaccounts.get_storage_accounts(
    ...
    def _parse_storage_account(self, raw_storage_account):
        ...
        storage_account[
            "https_traffic_enabled"
        ] = raw_storage_account.enable_https_traffic_only
        storage_account["public_traffic_allowed"] = self._is_public_traffic_allowed(
            raw_storage_account
        )
        storage_account[
            "trusted_microsoft_services_enabled"
        ...
        ] = self._parse_access_keys_last_rotation_date(
            raw_storage_account.activity_logs
        )
        ...
        for log in activity_logs:
            if (
                ...
                if (
                    ...
                    or last_rotation_date < log.event_timestamp
                ):

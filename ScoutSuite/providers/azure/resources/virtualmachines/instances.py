...
class Instances(AzureResources):
    ...
    async def _parse_instance(self, raw_instance):
        ...
        if raw_instance.availability_set is not None:
            ...
            try:
                instance_dict['availability_set'] = raw_instance.availability_set.id.split('/')[4] + ':' + \
                                                    raw_instance.availability_set.id.split('/')[8]
        ...
        if raw_instance.proximity_placement_group is not None:
            ...
            try:
                instance_dict['proximity_placement_group'] = raw_instance.proximity_placement_group.id.split('/')[
                                                                 4] + ':' + \
                                                             raw_instance.proximity_placement_group.id.split('/')[8]
            except Exception as e:
        ...
        if raw_instance.additional_capabilities is not None:
            ...
            instance_dict['additional_capabilities'] = [additional_capability for additional_capability, value in
                                                        raw_instance.additional_capabilities.__dict__.items() if
        ...
        if raw_instance.diagnostics_profile is not None:
            instance_dict['diagnostics_profile'] = {
                'Boot Diagnostics': True if raw_instance.diagnostics_profile.boot_diagnostics.enabled else None}
        ...
        if raw_instance.storage_profile is not None:
            ...
            if raw_instance.storage_profile.data_disks is not None and raw_instance.storage_profile.data_disks:
                instance_dict['storage_profile']['Data Disks'] = ["{} ({}GB)".format(disk.name, disk.disk_size_gb) for
                                                                  disk in raw_instance.storage_profile.data_disks]
        else:

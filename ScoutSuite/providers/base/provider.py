...
class BaseProvider(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.service_list = self._build_services_list(
            supported_services, services, skipped_services
        )
    ...
    def _build_services_list(supported_services, services, skipped_services):
        ...
        if error:
            print_info(
                "Available services are: {}".format(
                    str(list(supported_services)).strip("[]")
                )
            )
        ...
        return [
            ...
            if (services == [] or s in services) and s not in skipped_services
        ]
        ...
        for service in self.services:
            ...
            if self.services[service] is None:
                ...
                for finding in self.services[service]["findings"].values():
                    ...
                    last_run["summary"][service]["checked_items"] += finding[
                        "checked_items"
                    ]
                    last_run["summary"][service]["flagged_items"] += finding[
                        "flagged_items"
                    ]
                    items = finding.get("items", [])
                    if (
                        last_run["summary"][service]["max_level"] != "danger"
                        and len(items) > 0
                    ):
            ...
            for key in self.services[service]:
                if key != "regions_count" and key.endswith("_count"):
                    last_run["summary"][service]["resources_count"] += self.services[
                        service
                    ][key]
        ...
        for service_group in self.metadata:
            for service in self.metadata[service_group]:
                if service not in self.service_list:
                    continue
                if (
                    "hidden" in self.metadata[service_group][service]
                    and self.metadata[service_group][service]["hidden"] == True
                ):
                    continue
                ...
                for resource in self.metadata[service_group][service]["resources"]:
                    ...
                    if (
                        "full_path"
                        not in self.metadata[service_group][service]["resources"][
                            resource
                        ]
                    ):
                        self.metadata[service_group][service]["resources"][resource][
                            "full_path"
                        ] = self.metadata[service_group][service]["resources"][
                            resource
                        ][
                            "path"
                        ]
                    ...
                    if (
                        ...
                        self.metadata[service_group][service]["resources"][resource][
                            ...
                            [
                                x
                                for x in self.metadata[service_group][service][
                                    "resources"
                                ][resource]["full_path"].split(".")
                                if x != "id"
                            ]
                        )
                    ...
                    self.metadata[service_group][service]["resources"][resource][
                        "count"
                    ] = self.recursive_get_count(resource, self.services[service])
            ...
            for service in self.metadata[service_group]:
                ...
                if "summaries" in self.metadata[service_group][service]:
                    for summary in self.metadata[service_group][service]["summaries"]:
                        if (
                            summary == "external attack surface"
                            and service in self.services
                            and "external_attack_surface" in self.services[service]
                        ):
                ...
                for resource_type in self.metadata[service_group][service]["resources"]:
                    if (
                        "callbacks"
                        in self.metadata[service_group][service]["resources"][
                            resource_type
                        ]
                    ):
                        ...
                        callbacks = self.metadata[service_group][service]["resources"][
                            resource_type
                        ]["callbacks"]
                        self._new_go_to_and_do(
                            self.services[service], target_path, current_path, callbacks
                        )
                # Summaries
                if "summaries" in self.metadata[service_group][service]:
                    for summary in self.metadata[service_group][service]["summaries"]:
                        if (
                            "callbacks"
                            in self.metadata[service_group][service]["summaries"][
                                summary
                            ]
                        ):
                            current_path = ["services", service]
                            for callback in self.metadata[service_group][service][
                                "summaries"
                            ][summary]["callbacks"]:
                                callback_name = callback[0]
                                callback_args = copy.deepcopy(callback[1])
                                target_path = (
                                    callback_args.pop("path")
                                    .replace(".id", "")
                                    .split(".")[2:]
                                )
        ...
        for service_group in self.metadata:
            if "summaries" in self.metadata[service_group]:
                for summary in self.metadata[service_group]["summaries"]:
                    for callback in self.metadata[service_group]["summaries"][summary][
                        "callbacks"
                    ]:
                        callback_name = callback[0]
                        target_path = self.metadata[service_group]["summaries"][
                            summary
                        ]["path"].split(".")
                        ...
                        if callback_name == "merge":
                            for service in self.metadata[service_group]:
                                if service == "summaries":
                                    continue
                                if (
                                    "summaries" in self.metadata[service_group][service]
                                    and summary
                                    in self.metadata[service_group][service][
                                        "summaries"
                                    ]
                                ):
                                    try:
                                        source = get_object_at(
                                            self,
                                            self.metadata[service_group][service][
                                                "summaries"
                                            ][summary]["path"].split("."),
                                        )
    ...
    def _go_to_and_do(
        self, current_config, path, current_path, callback, callback_args=None
    ):
        """
        ...
        try:
            ...
            if key in current_config:
                ...
                for (i, value) in enumerate(list(current_config_key)):
                    if len(path) == 0:
                        if (
                            type(current_config_key == dict)
                            and type(value) != dict
                            and type(value) != list
                        ):
                            callback(
                                current_config_key[value],
                                path,
                                current_path,
                                value,
                                callback_args,
                            ...
                            callback(
                                current_config, path, current_path, value, callback_args
                            )
                    else:
                            ...
                            try:
                                ...
                                print_exception(
                                    e,
                                    {
                                        ...
                                        "current path": "{}".format(current_path),
                                        "key": "{}".format(
                                            key if "key" in locals() else "not defined"
                                        ),
                                        "value": "{}".format(
                                            value
                                            if "value" in locals()

...
class IAMFacade(GCPBaseFacade):
    ...
    async def get_bindings(self, project_id: str, service_account_email: str):
        try:
            ...
            print_exception(
                "Failed to retrieve service account IAM policy bindings: {}".format(e)
            )
            return []

    async def get_keys(
        self, project_id: str, service_account_email: str, key_types: list = []
    ):
        try:
            ...
            response = await run_concurrently(
                ...
                .serviceAccounts()
                .keys()
                .list(name=name, keyTypes=key_types)
                .execute()
            )
    ...
    async def get_service_accounts(self, project_id: str):
        try:
            ...
            return await GCPFacadeUtils.get_all(
                "accounts", request, service_accounts_group
            )
        except Exception as e:

...
class Keys(Resources):
    ...
    async def fetch_all(self):
        ...
        raw_keys = await self.facade.iam.get_keys(
            self.project_id, self.service_account_email, ["SYSTEM_MANAGED"]
        )
        ...
        raw_keys = await self.facade.iam.get_keys(
            self.project_id, self.service_account_email, ["USER_MANAGED"]
        )
        for raw_key in raw_keys:

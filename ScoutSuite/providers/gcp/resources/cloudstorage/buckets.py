...
class Buckets(Resources):
    ...
    def _parse_bucket(self, raw_bucket):
        ...
        policy = raw_bucket.iam_configuration.get('uniformBucketLevelAccess', False) or raw_bucket.iam_configuration.get('bucketPolicyOnly', False)
        if policy:
            ...
            raise 
        if not bucket_dict['uniform_bucket_level_access']:

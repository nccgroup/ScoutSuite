...
class DatabaseInstances(GCPCompositeResources):
    _children = [(Backups, "backups"), (Users, "users")]
    ...
    async def fetch_all(self):
        raw_instances = await self.facade.cloudsql.get_database_instances(
            self.project_id
        )
    ...
    def _parse_instance(self, raw_instance):
        ...
        instance_dict["has_failover_replica"] = (
            raw_instance.get("failoverReplica", []) != []
        )
        instance_dict["is_failover_replica"] = (
            raw_instance.get("masterInstanceName", "") != ""
        )
    ...
    def _set_last_backup_timestamps(self, instances):
        for instance_id, _ in instances:
            ...
            ] = self._get_last_backup_timestamp(self[instance_id]["backups"])

    def _get_last_backup_timestamp(self, backups):
        if not backups:
            return None
        last_backup_id = max(
            backups.keys(), key=(lambda k: backups[k]["creation_timestamp"])
        )

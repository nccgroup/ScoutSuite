...
class Keys(Resources):
    def __init__(
        self, facade: GCPFacade, project_id: str, keyring_name: str, location: str
    ):
    ...
    async def fetch_all(self):
        raw_keys = await self.facade.kms.list_keys(
            self.project_id, self.location, self.keyring_name
        )

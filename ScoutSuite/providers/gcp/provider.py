...
class GCPProvider(BaseProvider):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.services = GCPServicesConfig(
            self.credentials,
            self.credentials.default_project_id,
            self.project_id,
            self.folder_id,
    ...
    def _set_account_id(self):
        ...
        if self.all_projects:
            # Service Account
            if self.credentials.is_service_account and hasattr(
                    ...
                    for instance in zone["instances"].values():
                        for instance_disk in instance["disks"].values():
                            ...
                            for disk in project["snapshots"].values():
                                if (
                                    disk["status"] == "READY"
                                    and disk["source_disk_url"]
                                    == instance_disk["source_url"]
                                ):
                                    instance_disk["snapshots"].append(disk)

                            instance_disk["latest_snapshot"] = (
                                max(
                                    instance_disk["snapshots"],
                                    key=lambda x: x["creation_timestamp"],
                                )
                                if instance_disk["snapshots"]
                                else None
                            )

...
class TestOpinelUtilsConsoleClass(unittest.TestCase):
    ...
    def test_prompt_4_value(self):
        ...
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            display_choices=False, no_confirm=True, test_input='b') == 'b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            display_indices=True, no_confirm=True, test_input='1') == 'b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'], default='b', no_confirm=True, test_input='') == 'b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            no_confirm=True, authorize_list=True, test_input='a,b') == 'a,b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            required=True, no_confirm=True, test_input=['', 'b']) == 'b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            required=True, no_confirm=True, test_input=['invalid', 'b']) == 'b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'], no_confirm=True, test_input='a,c') is None
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            no_confirm=True, test_input='a,b', authorize_list=True) == 'a,b'
        assert prompt_value('prompt_4_value', choices=['a', 'b', 'c'],
                            no_confirm=True, test_input='a,e', authorize_list=True) is None

    def test_prompt_4_yes_no(self):
        assert prompt_yes_no('hello', 'N') == False
        assert prompt_yes_no('hello', 'no') == False
        assert prompt_yes_no('hello', 'Y')
        assert prompt_yes_no('hello', 'yes')
        assert prompt_yes_no('hello', ['foo', 'bar', 'no']) == False
        assert prompt_yes_no('hello', 'Ye') is None
        assert prompt_yes_no('hello', 'Non') is None
